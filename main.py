...
# # # Import components from your new files
...
# # # Import the API resource from its module
# # from google_calendar_api import api_resource

# # # Import voice I/O functions
...
# # # --- Chat Loop ---
# # chat_sessions: Dict[str, List[Any]] = {} # Dictionary to store chat history for different sessions
...
# #             chat_sessions[session_id] = current_chat_history
...
# from google_calendar_api import get_google_calendar_service, MockGoogleCalendarAPI
...
# if "api_resource" not in st.session_state:
#     try:
#         st.session_state.api_resource = get_google_calendar_service()
#         st.success("Google Calendar API service initialized successfully.")
#     except Exception as e:
#         st.error(f"Error initializing Google Calendar API service: {e}. Falling back to mock API.")
#         st.session_state.api_resource = MockGoogleCalendarAPI()
...
#         ),
#         StructuredTool.from_function(
#             func=partial(smart_schedule_meeting, api_resource=st.session_state.api_resource),
#             name="ScheduleMeeting",
#             description="Smartly schedules a meeting. If the preferred time is busy, it suggests the next available 30-min slot.",
#             args_schema=ScheduleMeetingInput,
#         ),
...
#         try:
#             result = await st.session_state.agent_executor.ainvoke({
#                 "input": user_input,
